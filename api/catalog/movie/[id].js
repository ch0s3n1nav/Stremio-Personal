...
module.exports = async (req, res) => {
  try {
    ...
    // For UFC events, we'll use a mock stream since they might not be real torrents
    if (id.startsWith('rd_ufc_')) {
      console.log('UFC event detected, using mock stream');
      
      const filename = parts.length >= 4 ? decodeURIComponent(parts.slice(3).join('_')) : 'UFC Event';
      let displayTitle = filename.replace(/\.(mkv|mp4|avi|mov|wmv|flv|webm|m4v|mpg|mpeg|ts|vob|iso|m2ts)$/i, '')
        .replace(/\./g, ' ')
        .replace(/_/g, ' ')
        .trim();

      // Mock stream for UFC events
      const streams = [
        {
          title: displayTitle,
          name: 'UFC Event Stream',
          url: 'https://example.com/ufc-stream.mp4', // Replace with actual UFC stream URL
          behaviorHints: {
            notWebReady: false,
            bingeGroup: `ufc-${id}`
          },
      ...
      console.log('Returning mock stream for UFC event');
      return res.json({ streams });
    }

    // Real-Debrid torrent handling for movies
